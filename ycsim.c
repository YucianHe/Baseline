...
int main(int argc, char *argv[]) {
			...
			else//isPCHit == 0
			{
				...
				if(flag == 1)//PC Read miss
				{
					...
					if(CheckSSDTable(page_no, flag) == 1)//3. Read Hit in SSD
					{
						...
						fprintf(result, "3.Read miss in Page Cache but hit in SSD!\n");
					...
					else//4. Read Miss in SSD
					{
						...
						fprintf(result, "4.read miss in Page Cache and miss in SSD too!\n");
	...
	*/
	...
	printf("[Trace Reading Finish-----Basic]\n\n");
